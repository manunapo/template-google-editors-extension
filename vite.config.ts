...
const DEV_SERVER_URL =
  process.env.DEV_SERVER_URL || `https://localhost:${PORT}`;
...
// Check if we're using a tunnel (not localhost)
const isUsingTunnel = !DEV_SERVER_URL.includes('localhost');

const devServerOptions: ServerOptions = {
  port: PORT,
  host: true, // Listen on all addresses, needed for tunnels
  cors: true, // Enable CORS for cross-origin requests
  headers: {
    // Required for Private Network Access preflight requests
    'Access-Control-Allow-Private-Network': 'true',
  },
};

// Only use local HTTPS when NOT using a tunnel (tunnel handles SSL termination)
if (!isUsingTunnel) {
    ...
    targets.push(
      ...clientEntrypoints.map((entrypoint) => ({
        ...
        transform: (contents: string) =>
          contents
            .toString()
            .replace(/__DEV_URL__/g, DEV_SERVER_URL)
